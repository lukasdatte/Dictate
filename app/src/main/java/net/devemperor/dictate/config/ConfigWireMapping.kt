 ...
 * (`AIProvider`, `AmbiguityMode`, `PromptMode`) and the platform-free **wire** enums in `:shared`
 * (`ProviderType`, `AmbiguityModeValue`, `PromptSelectionMode`). `:shared` sits below `:shared-ai`
 * and cannot reference the domain originals, so it defines its own mirror enums (spec §4.8, §13 D6).
 *
