...
- **Related Plans:**
  - [dictate-keyboard-layout-refactor](../plans/2026-05-07%20-%20dictate-keyboard-layout-refactor/dictate-keyboard-layout-refactor.reviewed.md) §4.0.1.1, §4.0.1.2, §4.0.1.6, §4.0.5 — the plan that motivated this ADR.
  - [dictate-cutover-completion](../plans/2026-05-15%20-%20dictate-cutover-completion/dictate-cutover-completion.md) — the Epic that completed the single-dispatch cutover (collapsed the two-orchestrator coexistence; see Decision History 2026-05-17). §8 of that plan references this ADR (bidirectional).
- **Related Spec:** [Spec 1 — Pipeline-Service](../plans/2026-05-07%20-%20dictate-keyboard-layout-refactor/research/1-pipeline-service/1-pipeline-service.reviewed.md) §3, §4.1, §4.2, §4.3, §4.4, §4.8, §15, §9.6 (PipelineRunnerSubsystem adaptee boundary)
- **Related ADRs:**
...
### 2026-05-17 — Two-orchestrator coexistence collapsed; single-dispatch is now the production recording driver (Epic dictate-cutover-completion)

**Trigger:** Parent-plan Phase-4 Integration Check INT-1 (`Critical, escalate-to-user`) found the new `DictateOrchestrator` was a *parallel-dormant* layer — built but not driving production recording. The user-decided follow-up Epic `dictate-cutover-completion` (`docs/plans/2026-05-15 - dictate-cutover-completion/dictate-cutover-completion.md`) executed the cutover. Resolution code-verified in `reports/integration-check.md` Central Verdict §1/§2 (INT-1 RESOLVED).

**Before:** Two-orchestrator coexistence. `DictateOrchestrator` was a registry-driven state-router that was nominally in place but **not** the production driver for recording: `ModuleServices.pipelineRunner` and `.notificationCoordinator` were `Log.w` no-op stubs (`PipelineServiceStubSubsystems`), and the IME drove production recording via the legacy `JobExecutor.INSTANCE.start` call-sites. ADR-0001's single-dispatch existed but the recording axis bypassed it in production.

**After:** The coexistence is **collapsed** — `DictateOrchestrator` is the sole state-router for production recording. `DictatePipelineService.onCreate` Step 3/4 constructs and binds the **real** adapters: `PipelineRunnerSubsystemAdapter` (a thin `JobExecutor.start`-delegating `PipelineRunnerSubsystem`, Spec 1 §9.6 OQ-1 thin-delegation) and `PipelineNotificationCoordinator`; the stubs are demoted to `@Deprecated(WARNING)` test-only `val`s. The IME dispatches `RecordingAction.StartRecording` / `StopRecordingAndSend` (single-dispatch per AC-10); the legacy `JobExecutor.INSTANCE.start` recording-trigger sites and the `USE_LEGACY_RECORDING_DRIVE` guarded fallback are deleted (one documented RESUME carve-out survives, regression-locked by `CutoverArchitectureInvariantTest`). New seams added under this pattern: `ImePipelineConfigResolver` (R-1 IME-config-fidelity seam) and `preferences/LanguageResolver` (D-13 — the deleted `LanguageController` split into a stateless resolver + `LanguageModule`). State-shape additions stay within existing axes (no new module): `RecordingState.sessionId` (F-10 source), the `ReprocessStaging→Preparing` single-submit FSM guard with no `isStarting` field (F-12 option-b), `Running.completedSteps/totalSteps/elapsedMs` (F-13), language-aware `dictateButtonText` (F-15). The module axis-count is **unchanged** (13 active + 1 Phase-2 stub) — `PipelineOrchestrator`/`JobExecutor` survive only behind the `PipelineRunnerSubsystem` interface as the Spec 1 §9.6 adaptee, never as a second live state-router.

**Reasoning:** Per D7 (long-term-quality) a permanent parallel-dormant layer is two implementations of recording forever — the exact anti-pattern the modular-orchestrator pattern exists to kill. INT-1 routing option (a) (implement the parent ADRs' intended end-state) was chosen over (b) (accept-dormant); this **fulfils** ADR-0001's single-dispatch + single-owner-per-axis decision rather than introducing a new paradigm — so it is a Decision-History append, **not** a new ADR and **not** a supersede (`## Supersede Triggers` reserves superseding for a *different* state-mutation paradigm). The cross-cutting lesson is the load-bearing point: the INT-1 parallel-dormant anti-pattern **recurred three times** during the cutover — render-layer (C10-IMPL-2), edit-bar/emoji listener-bundle (CR4-IMPL-1), and the RESEND-action staging-override (CR4-IMPL-3 / F-2·F-6) — and each recurrence was **resolved, not re-deferred**, behind the §6.2 staged build-but-dormant→atomic-flip safety-net, with the mandatory RR-3 per-class responsibility-trace gating each deletion and `CutoverArchitectureInvariantTest` (INT-3) locking the invariant against silent regression. This is the direct consequence of ADR-0001's single-dispatch + D7, which is why no standalone "no parallel-dormant layer" ADR is created — it would restate D7 and this ADR's single-dispatch.

**Reference:** `docs/plans/2026-05-15 - dictate-cutover-completion/dictate-cutover-completion.md` (Epic); `docs/plans/2026-05-15 - dictate-cutover-completion/reports/integration-check.md` (INT-1 RESOLVED Central Verdict §1/§2, INT-3 regression-lock); `docs/plans/2026-05-15 - dictate-cutover-completion/research/render-path-cutover.md`; `docs/plans/2026-05-15 - dictate-cutover-completion/research/sendstaging-isstarting-guard-semantics.md` (F-12 option-b rationale).
