...
- **`KeyboardLayoutManager`**: zentraler Triangle-FSM-Owner. Subscribiert an `DictateUiState` (aus Spec 1), entscheidet über aktiven LayoutMode + RenderBackend.
...
sealed class Action {
    ...
    object NoOp : Action()    // Default für nicht-bindene Slots

    // ─── Recording-Achse (RecordingModule) ───
    sealed class RecordingAction : Action() {
        data class StartRecording(val target: InsertionTarget) : RecordingAction()
        object MediaRecorderReady : RecordingAction()    // Hardware-Callback
    ...
    sealed class PipelineAction : Action() {
        data class TriggerPipeline(val audioFile: java.io.File) : PipelineAction()
        data class StartPipeline(val totalSteps: Int, val autoEnterActive: Boolean) : PipelineAction()
        data class StepStarted(val stepName: String) : PipelineAction()
        object StepCompleted : PipelineAction()
        data class StepFailed(val reason: String) : PipelineAction()
        data class PipelineDone(val finalText: String, val sessionId: Long) : PipelineAction()
        data class PipelineFailed(val reason: String, val sessionId: Long) : PipelineAction()
        object CancelPipeline : PipelineAction()
        data class StartReprocessStaging(val sessionId: Long) : PipelineAction()
        data class UpdateReprocessQueue(val newQueue: List<Int>) : PipelineAction()
        data class UpdateReprocessLanguage(val code: String?) : PipelineAction()
        object SendStaging : PipelineAction()
        object CancelReprocessStaging : PipelineAction()
        data class ConfirmInsertion(val sessionId: Long) : PipelineAction()
    }
    ...
    sealed class PendingSessionsAction : Action() {
        ...
        data class Dismiss(val sessionId: Long) : PendingSessionsAction()
    }
...
class ImeViewBackend(
    ...
    override fun render(state: DictateUiState, mode: LayoutMode) {
        ...
        // 1. MotionLayout-Transition zum richtigen Scene-State
        val targetSceneState = mode.id.toSceneStateId()
        if (state.animationsEnabled) {
            motionLayout.transitionToState(targetSceneState)
        } else {
            motionLayout.jumpToState(targetSceneState)
        }
    ...
    private fun wireStaticHandlers() {
        buttonViews.forEach { (id, view) ->
            view.setOnClickListener {
                ...
                onAction?.invoke(slot.actionResolver(s))
            }
            keyPressAnimator.applyPressAnimation(view)
        }

        // Special long-click handlers
        buttonViews[LogicalButtonId.RECORD]?.setOnLongClickListener {
            onVibrate(); onAction?.invoke(Action.NoOp); true   // konkrete Long-Action via Slot
        }
        buttonViews[LogicalButtonId.RESEND]?.setOnLongClickListener {
            onVibrate(); onAction?.invoke(Action.ResendLastAudioLong); true
        }
        buttonViews[LogicalButtonId.BACKSPACE]?.setOnLongClickListener {
            onAction?.invoke(Action.NoOp); true
        }
...
`PropertySet motion:visibilityMode="ignore"` wird auf jeden Button gesetzt, dessen Visibility durch den `LayoutCatalog` und nicht durch die MotionScene gesteuert wird. Dies sind alle Buttons, die in mindestens einem LayoutMode `visibilityPredicate` mit nicht-konstantem Wert haben:

| Button | Predicate-abhängig? | `visibilityMode="ignore"` nötig? |
|--------|---------------------|----------------------------------|
| record_pulse_layout | nein (immer visible) | nein |
| resend_btn          | ja (Idle + lastAudio + ResendPref + !PipelineActive) | **JA** |
| backspace_btn       | nein (immer visible) | nein |
| audio_focus_btn     | ja (nur in Single-Row sichtbar) | **JA** |
| trash_btn           | ja (Recording / ReprocessStaging) | **JA** |
| space_btn           | nein (immer visible) | nein |
| pause_btn           | ja (Recording / ReprocessStaging) | **JA** |
| enter_btn           | nein (immer visible) | nein |

→ 4 Buttons brauchen `visibilityMode="ignore"`. In der Scene oben sind sie gesetzt.
    ...
    rows = listOf(
        ...
        RowDescriptor(slots = listOf(
            ...
            ButtonSlot(LogicalButtonId.SPACE, FillRemaining,
                visibilityPredicate = { true },
                actionResolver = { Action.SpaceKey }),
            ...
            ButtonSlot(LogicalButtonId.ENTER, WrapContent,
                visibilityPredicate = { true },
                actionResolver = { Action.EnterKey }),
        )),
    ),
)
```
...
val KEYBOARD_SINGLE_ROW_SEND_MODE = LayoutMode(
    ...
    rows = listOf(RowDescriptor(slots = listOf(
        ...
        ButtonSlot(LogicalButtonId.SPACE, FillRemaining,
            visibilityPredicate = { true }, actionResolver = { Action.SpaceKey }),
        ButtonSlot(LogicalButtonId.PAUSE, WrapContent,
            visibilityPredicate = { false }, actionResolver = { Action.NoOp }),
        ButtonSlot(LogicalButtonId.BACKSPACE, WrapContent,
            visibilityPredicate = { true }, actionResolver = { Action.Backspace }),
        ButtonSlot(LogicalButtonId.ENTER, WrapContent,
            visibilityPredicate = { true }, actionResolver = { Action.EnterKey }),
        ButtonSlot(LogicalButtonId.RESEND, WrapContent,
            visibilityPredicate = { false }, actionResolver = { Action.NoOp }),
        ButtonSlot(LogicalButtonId.AUDIO_FOCUS, WrapContent,
            visibilityPredicate = { true }, actionResolver = { Action.ToggleAudioFocus }),
    ))),
)
```
    ...
    rows = listOf(
        RowDescriptor(slots = listOf(
            ButtonSlot(LogicalButtonId.RECORD, FillRemaining,
                ...
                actionResolver = { Action.SendStaging }),
            ButtonSlot(LogicalButtonId.RESEND, WrapContent,
                visibilityPredicate = { false },
                actionResolver = { Action.NoOp }),
            ButtonSlot(LogicalButtonId.BACKSPACE, WrapContent,
        ...
        RowDescriptor(slots = listOf(
            ButtonSlot(LogicalButtonId.TRASH, WrapContent,
                visibilityPredicate = { true },
                actionResolver = { Action.CancelReprocessStaging }),
            ButtonSlot(LogicalButtonId.SPACE, FillRemaining,
                visibilityPredicate = { true },
                actionResolver = { Action.SpaceKey }),
            ButtonSlot(LogicalButtonId.PAUSE, WrapContent,
...
fun resolveRecordAction(state: DictateUiState): Action = when (state.recording) {
    is RecordingState.Idle      -> Action.StartRecording
    is RecordingState.Active    -> Action.StopRecordingAndSend
    is RecordingState.Paused    -> Action.StopRecordingAndSend
...
fun resolveRecordActionPipeline(state: DictateUiState): Action = when (state.pipeline) {
    is PipelineUiState.Running -> Action.NoOp   // Click toggelt AutoEnter — separate Slot-Aktion möglich
    else -> Action.NoOp
}

fun resolveTrashAction(state: DictateUiState): Action = when {
    state.pipeline is PipelineUiState.ReprocessStaging -> Action.CancelReprocessStaging
    else                                               -> Action.CancelRecording
}

fun resolvePauseAction(state: DictateUiState): Action = when (state.recording) {
    is RecordingState.Paused -> Action.ResumeRecording
 ...
 * Code-Pfade, die `state.audioFocusEnabled` in ein Icon mappen — eine Drift-Quelle.
 *
 ...
 * (in `refreshAudioFocusIcon(state.audioFocusEnabled)`) referenzieren ihn.
 */
...
object LayoutCatalog {
    fun forKeyboard(state: DictateUiState): LayoutMode {
        ...
        return when {
            ...
            isPipelineLive && !state.singleRowMode     -> KEYBOARD_TWO_ROW_SEND_MODE
            !isPipelineLive && state.singleRowMode     -> KEYBOARD_SINGLE_ROW
...
| Preparing-State + SingleRow-Toggle | Heute: KSM.refresh wird durch SingleRowToggle NICHT getriggert (Bug-Klasse "SSOT-Lücke" in §7 von _pending-state-machine-visibility-owners.md) | Im Refactor: `state.singleRowMode` ist Teil des DictateUiState, jeder Toggle erzeugt eine neue State-Emission → Manager re-rendert komplett | Bug eliminiert |
...
```kotlin
val targetSceneState = mode.id.toSceneStateId()
if (state.animationsEnabled) motionLayout.transitionToState(targetSceneState)
...
| Heute | Ziel |
|---|---|
| `MainButtonsController.kt:76-79` (`recordClickListener` → `callback.onRecordClicked()`) | `ImeViewBackend.wireStaticHandlers` (RECORD-Slot → `actionResolver(state) = Action.StartRecording / StopRecordingAndSend / …`) |
...
### §9.3 KeyboardStateManager.applyVisibility → entfällt

| Heute | Ziel |
|---|---|
| `KeyboardStateManager.kt:158-169` (`applyVisibility()`) | gelöscht — Catalog-Predicates übernehmen |
| `KeyboardStateManager.kt:171-181` (`applyContentAreaVisibility`) | bleibt — wird zu einem orthogonalen `ContentAreaController`. Touchen ContentArea-Container, nicht Buttons. |
| `KeyboardStateManager.kt:183-192` (`applyRecordingControlsVisibility`) | gelöscht — Predicates in `predTrashVisible` / `predPauseVisible` (§8.5) |
| `KeyboardStateManager.kt:194-224` (`applyPromptsVisibility`) | bleibt — Promptbar ist nicht Teil der Main-Button-Area, eigene Achse |
| `KeyboardStateManager.kt:162` (`overlayCharactersLl.visibility = GONE` reset) | bleibt — overlay-spezifisch |

**Nettogewinn:** ~10 Zeilen (`applyRecordingControlsVisibility`) gelöscht, ContentArea und Prompts bleiben.
...
| `KeyboardUiController.kt:147-155` (`updateDictateUiState`) | gelöscht — DictateUiState wird im PipelineStateManager (Spec 1) gehalten und reactive emittiert |
...
- [ ] Send-Mode + Single-Row: Send-Button vollständig sichtbar, kein Verdecken (Bug-Eliminierung).
...
**Amplitude/Timer-Hooks:** kommen direkt vom `PipelineStateManager` (Spec 1) — nicht über `DictateUiState`-Emission, weil das pro-Tick zu neuen State-Allocations führen würde. Stattdessen: eigener `StateFlow<AmplitudeTick>` oder simple Callback-Methode am LocalBinder.
...
**Risiko 2:** Wenn ein `state`-Snapshot in einem Lambda gefangen wird, das vom System verzögert ausgeführt wird (z.B. Touch-Event in der Frame-Queue), zeigt der Click die `actionResolver` von einem **veralteten** State an. Bei Pipeline-Tick-Race: Click auf record_btn könnte `Action.NoOp` (alter Pipeline.Running-State) statt `Action.StartRecording` (neuer Idle-State) emittieren.
...
private fun buildBackspaceSwipeHandler(): View.OnTouchListener =
    BackspaceSwipeHandler(
        ...
        onDeleteCancelled = { /* siehe MainButtonsController:189-193 — wird vom Service gehandled,
                                 ggf. via `onAction(Action.NoOp)` als Marker */ },
...
[Block 4] Manager + Catalog (parallel zu Block 5 möglich, aber reine Daten)
   ...
   │
   └─► [Block 5d] Cleanup: KeyboardLayoutModeController, MainButtonsController-
         Click-Logic, KSM.applyRecordingControlsVisibility, RecordingUiController.applyXxxState
         löschen. **Nur gemacht, wenn 5c funktioniert!**
```

**Begründung der Reihenfolge:**
- 5a/5b parallelisierbar, da sie auf Mock-State testbar sind.
- 5c muss vor 5d kommen, sonst gibt es eine Phase, in der weder altes noch neues System Visibility setzt.
- 5d ist destruktiv — strikt am Ende.
...
| `recordButton.setOnLongClickListener` (Z. 163) | wireStaticHandlers (RECORD long-click) → Action.NoOp (heutiges `onRecordLongClicked` ist Settings-Open; kann separater Slot oder ein `recordLongAction` werden) |
| `resendButton.setOnClickListener` (Z. 170) | `RESEND.actionResolver` → Action.ResendLastAudio |
| `resendButton.setOnLongClickListener` (Z. 174) | wireStaticHandlers (RESEND long-click) → Action.ResendLastAudioLong |
| `backspaceButton.setOnClickListener` (Z. 181) | `BACKSPACE.actionResolver` → Action.Backspace |
...
| `audioFocusButton.setOnClickListener(audioFocusClickListener)` (Z. 242) | `AUDIO_FOCUS.actionResolver` → Action.ToggleAudioFocus |
...
| `enterButton.setOnClickListener` (Z. 245) | `ENTER.actionResolver` → Action.EnterKey |
...
| `editNumbersButton.setOnClickListener` (Z. 102) | **BLEIBT** in EditBar-Controller — Action.ToggleSmallMode wird emittiert |
| `editNumbersButton.setOnLongClickListener` (Z. 112) | **BLEIBT** in EditBar-Controller — Action.ToggleSingleRowMode |
| `editSettingsButton/editHistoryButton/pipelineCancelBtn` (Z. 118-120) | **BLEIBT** in EditBar-Controller — separate Achse |
| `editKeyboardButton` (Z. 126/131) | **BLEIBT** in EditBar-Controller — Action.ToggleQwertz |
...
**Mögliche weitere DRY-Optimierung (zu validieren durch Spike):** statt 4 LayoutMode-Konstanten (TWO_ROW, SINGLE_ROW, TWO_ROW_SEND, SINGLE_ROW_SEND) ein einziger `LayoutMode` mit konditionalen Predicates, der `state.singleRowMode` und `state.pipeline` direkt liest. **Trade-off:** weniger Konstanten, aber komplexere Predicates pro Slot. Empfehlung: **bleibt bei 4 Konstanten** — Lesbarkeit > Code-Compactness, und die DRY-Pflicht ist bereits durch geteilte Predicate/Resolver-Funktionen erfüllt.
...
| `resolveTrashAction` | TRASH-Slot in TWO_ROW + SINGLE_ROW (REPROCESS_STAGING hat eigenen `Action.CancelReprocessStaging`-Direkt-Wert, weil semantisch festgelegt) |
...
**Gap 1: Edit-Bar-Audio-Focus-Btn (`edit_audio_focus_btn`) ist nicht im LayoutCatalog**
...
**Gap 2: `setResendEnabled(false)` Cooldown nach Resend-Klick (500ms)**
...
**Gap 3: ContentArea-Achse vs. LayoutCatalog**
...
**Gap 4: BorderGlow-Animation während Pipeline (Send-Mode)**
...
**Gap 5: Migration-Reihenfolge versus Hauptplan-Block-Reihenfolge**
...
- **UI-Test 1:** Toggle Single-Row im Idle. Verify alle 8 Buttons sichtbar, Layout entspricht Single-Row-State.
- **UI-Test 2:** Recording starten → Verify resend GONE, trash/pause VISIBLE.
- **UI-Test 3:** Recording-Stop → Pipeline → Verify record_btn-Text ist "Sende…" / "1/3 0:01" (Counter), trash/pause GONE.
- **UI-Test 4:** Send-Mode + Single-Row → Verify Send-Button vollständig sichtbar (kritischer Bug-Fix-Verifikator).
- **UI-Test 5:** ReprocessStaging → Verify pause_btn VISIBLE+disabled+alpha 0.4.
- **UI-Test 6:** Re-Inflate (Rotation) während Recording → Verify Animation läuft weiter, korrekter LayoutMode auf erstem Frame.
- **UI-Test 7:** Toggle Single-Row während Recording → Verify Pulse-Animation läuft durch, Send-Btn-Position wechselt korrekt.
