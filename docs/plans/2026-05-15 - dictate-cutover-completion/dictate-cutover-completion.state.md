...
**Theme-C-R chunks (Block B5 — render-path cutover; sequential CR1→CR2→CR3→CR4→CR-RGATE→CR-DEL):**

| # | ID | Block | Chunk-Name | Risk | Status | Agent-IDs | Impl-Commit | Test-Commit |
|---|-----|-------|------------|------|--------|-----------|-------------|-------------|
| CR1 | CR1-RR1 | B5 | ImeViewBackend completion (Spec 2 §6/§9.2; A1 long-press) | Med | ✅ | B5-CR1-IMPL | 3d8b9f0 | 598534b |
| CR2 | CR2-RR2 | B5 | special-touch-installer (CursorSwipe/Backspace/Enter) | **HIGH** (RR-1) | ✅ | B5-CR2-IMPL | d53208c | 590049e |
| CR3 | CR3-RR3 | B5 | visibility-controller-attach + KSM-thinning | **HIGHEST** (RR-2/A3) | ✅ | B5-CR3-IMPL | f78af84 | 5765432 |
| CR-EXT | CR-EXTRACT | B5 | EditBar/Emoji/OverlayChars owner extraction (CR4-IMPL-1 fix) | Med | ✅ (mid-triage W1) | B5-CR4-MID-RES-1 | bd74258 | bd74258 |
| CR4 | CR4-RR4 | B5 | IME legacy-driver-removal (render AC-10 analogue) | **HIGHEST** (RR-1/2) | ✅ (re-run) | B5-CR4-IMPL | e1e754b | b5e224e |
| CR-RGATE | CR-RGATE | B5 | **render verification GATE** → **GREEN** (CR-DEL authorised) | Gate | ✅ GREEN | B5-CR-RGATE-IMPL | — | 282604b |
| 10 | C10-C3 (CR-DEL) | B5 | 4-controller deletion + edit-theme/G9/G13 extract + F-6 | Med | ✅ | B5-C10-C3-IMPL | cc5803e | 0c2c196 |

| 11 | C11-D1 | B6 | Espresso UI-Tests 1-10 + Robolectric mirror | D | Low-Med (R-6) | ✅ | B6-C11-D1-IMPL | (empty) | 397bfbd |
| 12 | C12-D2 | B6 | final integration E2E + cleanup-grep regression | D | Low | ✅ FINAL-LOCK GREEN | B6-C12-D2-IMPL | (none — verification) | (none) |
...
|-------|------|----------|----------|---------|------------|
| CR4 | step-1-impl | CR4-IMPL-1 | Critical (architecture-conflict, blocks-following-chunks) | B5-CR4-MID-W1 (iter 1/cap 2) | research+repair: registerAllListeners() bundled edit-bar/emoji/overlay-chars (3 sub-axes Spec2 §13.2 says BLEIBT in EditBarController/EmojiController — classes never created; INT-1 pattern 3rd recurrence). CR-EXTRACT: 3 Kotlin owners extracted build-but-dormant per §13.2 (verbatim for EditBar/Emoji; OverlayChars proposed D22). CR4-IMPL-2 (G8 cooldown dispatch) fixed too. chunks.json: CR-EXTRACT inserted before CR4. ✓ converged, CR4 re-run authorised. Wave-commit bd74258 |
...
| B6 (Theme D — test-completion) | cef46be | d6a1a84 | ✅ (C11-D1 Espresso 1-10 + Robolectric mirrors AC-8; C12-D2 FINAL-LOCK GREEN all AC-1..10; VAL-W1 1 trivial doc 🟢; 1172/0 both variants; **ALL 6 Epic blocks done — implementation CODE-COMPLETE + verification-locked**) |
| B5 (Theme C-R — render-path cutover) | c92ebd1 | 4bcd1a7 | ✅ (CR1/CR2/CR3/CR-EXT/CR4/CR-RGATE-GREEN/CR-DEL; 2 mid-triage [CR4-IMPL-1→CR-EXTRACT, render-flip] + VAL-W1 1 wave [2 Crit: SPACE-double-commit + F-6-reopened-closed]; INT-1 anti-pattern caught 3× & resolved; 4 controllers deleted; 1162/0 both variants; last R-7 axis closed) |
...
|-----------|-------|--------|---------|
| 2026-05-17 | Phase 5b/5c wave 2 + Phase 5 closure | PHASE5-TRANSLATE wave 2 (4 parallel translators, disjoint files): the parent plan's genuinely-German **plan file (split per D16 into 3 `.en.md`) + 3 specs (~14,200 lines of dense technical German)** all translated 1:1 and parity-verified (heading-count, fenced-block-count, code-block byte-identity each confirmed per file). **Parent EN-sidecar deliverable now CLOSED — 0 outstanding** (supersedes the prior-row "tracked outstanding" note). Disposition records updated consistent across both per-plan READMEs + `docs/plans/README.md` + parent state-file `plan_lifecycle.en_translation` (`outstanding-genuine-german: []`, status CLOSED). Phase-5 wave committed. **Epic + parent plan fully archived & closed.** Self-check ✅. | ✅ |
| 2026-05-17 | Phase 5b/5c wave 1 | PHASE5-TRANSLATE: language audit + EN-sidecar disposition (D4). **This Epic's plan + all 4 research files are `english-native` (English prose; German limited to a few §-headings).** Per D4 / SSoT / `language-conventions.md` (the German-working-language → EN trigger does not apply), **no `.en.md` produced for this Epic — correct, not skipped.** A near-verbatim sidecar would be the redundant duplication the SSoT rule forbids. Disposition recorded in this Epic's per-plan README "Language Disposition (Phase 5b/5c)". (Parent plan `dictate-keyboard-layout-refactor` is genuinely German-native — wave 1 translated 6 of its German research files; the plan file + 3 specs were honestly recorded as tracked-outstanding here and closed by wave 2 — see the row above.) Self-check ✅. | ✅ |
...
| 2026-05-17 | Phase 3.1 B6-C12-D2 | B6-C12-D2-IMPL: the FINAL holistic verification lock (post-all-blocks, fully-cutover legacy-FREE path; verification-only, zero prod/test delta). **FINAL-LOCK: GREEN — all AC-1..AC-10 PASS.** 1172/0/0 debug + 1172/0/0/16-skip release, byte-identical ×2 uncached different-order; +226 net vs 946 baseline; no R-7 flake (all 3 axes closed). Every grep verdict clean: AC-1 stub comment-only, AC-5/AC-6 zero, AC-7 4 controllers deleted zero-code-ref, AC-10 exactly 1 JobExecutor.start (RESUME carve-out) + zero USE_LEGACY_RECORDING_DRIVE + PipelineOrchestrator adapter-only (Spec1 §9.6). Keystone F-1/F-2/F-3 + Triangle T1-T7: DictateCutoverE2ETest 10/10 + RenderPathCutoverGateTest 5/5 green. Dev-1 no 3rd test file (would be vacuous duplication, RR-4). Epic implementation CODE-COMPLETE + verification-locked. Phase-4.5 manual device-tier runbook noted out-of-scope for this auto-lock. No commit (verification). | ✅ |
| 2026-05-17 | Phase 3.1 B6-C11-D1 | B6-C11-D1-IMPL combined Steps 1-5 (Theme-D, pure test chunk): 10 Espresso UI-1..10 bodies per Spec2 §14.2 (un-@Ignore'd, compile-proven androidTest) + 10 Robolectric mirrors (green, the OQ-4 CI path; device infra unavailable R-6). Both drive the real post-cutover sole render path (legacy controllers deleted). AC-8 met. ZERO src/main change (single test-commit). Dev-1 UI-1 subset-present (WIDGET_TOGGLE post-§14.2); Dev-2 UI-3 deterministic test-formatter. 1212 tests green, CR-RGATE 5/5 + DictateCutoverE2ETest 10/10 stay green. No blockers. Commit 397bfbd. | ✅ |
| 2026-05-17 | Phase 3.2 B5 | Block-Validate B5 (largest block — Theme-C-R render-cutover). Note: 3 audits crashed on transient API-500 (no output) → all 4 re-spawned fresh. 4-topic → 2 Crit / ~6 Imp / ~7 NTH → VAL-SANITY 11 unique → 🟢9/🟡2/❌4. B5-VAL-W1: F-1 SPACE-double-commit (real regression) fixed option-(i); F-2 F-6-collapse-incomplete → F-6 RE-OPENED+properly-closed (dispatchStagingOverride 4 boundaries); F-3 edit-bar-audio-focus-icon §13.2-F-4; **last R-7 axis closed** (JobExecutor.resetForTest sentinel-drain — flake GONE). 6 more 🟢; ❌4 accepted. 1162/0 both variants; CR-RGATE+E2E stay green. Wave-commit 4bcd1a7. **Block B5 ✅ COMPLETE — render-path cutover fully done+validated; the INT-1 anti-pattern was caught & spec-faithfully resolved 3× across the Epic.** | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR-DEL | CR-DEL (point-of-no-return, CR-RGATE-GREEN-authorised). RR-3 per-class trace PASSED (4 classes) — surfaced 4 live gaps each resolved pre-deletion: G13→new PipelineStepRowRenderer, G9→new QwertzRecordingController, edit-row-theme→EditBar/Emoji.applyTheme, 4th ContentArea axis→ContentAreaController. F-6 CLOSED (single LanguageState.override carrier). Deleted MainButtonsController/RecordingUiController/KeyboardUiController/KeyboardStateManager + IME refs + KeyboardUiControllerTest; PipelineOrchestrator kept (Spec1 §9.6). AC-RR-7 grep-zero (assembleDebug+Release green=proof). AC-RR-8 = Epic AC-7+AC-10 render half. BOTH variants 1153/0/0 (net +18); CR-RGATE+E2E proofs stay green with controllers gone; R-7 PipelineRunner flake did NOT fire. No Critical → no mid-chunk-triage. F-6 + CR4-IMPL-3 closed; C10-C3-IMPL-1 NTH open. **All B5 Theme-C-R impl chunks done — render-path cutover CODE-COMPLETE.** Commit 1 cc5803e + Commit 2 0c2c196. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR-RGATE | Render verification GATE (C6/D2-pre analogue). New RenderPathCutoverGateTest 5/5 deterministic on real bound binder (RR-4-mitigated). Auto-tier all uncached: assembleDebug green; debug ×2 1130/0/0; release 1130/0/0 (R-7 flake didn't fire); +gate 1135/0/0. G2-G16 all fire via new owner; doubleWriteCount==0, new owners sole live-writer (flip complete); CR4 SetContentArea proven (no blank). Theme-residual (1 un-guarded bound-path mainButtonsController.applyTheme ~11 edit-row btns) ruled NON-BLOCKING (non-state axis; CR-DEL's grep-zero scope; deletion=loud compile error). CR4-IMPL-4=spec-target; F-6=CR-DEL-scoped. **RENDER-GATE: GREEN — CR-DEL AUTHORISED.** Commit 282604b. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR4 (re-run) | CR4-IMPL re-run post CR-EXTRACT: the render-flip done. Per-axis atomic via bound/unbound exclusive switch (CR1-CR3 pattern); legacy mainButtonsController/stateManager drive removed + CR1-CR3/CR-EXTRACT owners armed same pass; legacy classes instantiated-but-dead. CR4-IMPL-1 did NOT recur. Critical catch: contentArea not pref-mirrored → +LayoutAction.SetContentArea dispatch (RR-2 avoided). A3 extraction correctly staged to CR-DEL (followed validated plan SoT over prompt wording, D6). CR4-IMPL-3 (RESEND-action no new-path impl, same §13.2 pattern) resolved INLINE via imeSideAffordance (accepted A1-class mechanic, not re-escalated). CR4-IMPL-2 legacy setResendEnabled removed. Theme-edit-row residual + CR4-IMPL-4 → flagged for CR-RGATE. F-6 → CR-DEL. 1130 tests green (testRelease PipelineRunner flake = pre-existing R-7 → B5 AUDIT-TEST). No mid-chunk-triage. Commit 1 e1e754b + Commit 2 b5e224e. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR4 (blocked→triage) | CR4-IMPL combined Step1 found Critical CR4-IMPL-1 (architecture-conflict): registerAllListeners() bundles 3 no-owner sub-axes (edit-bar/emoji/overlay-chars) Spec2 §13.2 says BLEIBT in non-existent EditBarController/EmojiController — INT-1 pattern 3rd recurrence. Agent correctly halted (no source). mid-chunk-triage B5-CR4-MID-W1: CR-EXTRACT extracted 3 build-but-dormant Kotlin owners per §13.2 (EditBar/Emoji verbatim; OverlayChars D22) + fixed CR4-IMPL-2 (ResendCooldownExpired dispatch). chunks.json amended (CR-EXTRACT before CR4). Live keyboard unchanged, 1129 tests green. ✓ converged → CR4 re-run authorised. Wave-commit bd74258. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR3 | B5-CR3-IMPL combined Steps 1-5 (Theme-C-R, HIGHEST RR-2/A3): 3 R.10 controllers attach via attachBackend (backendType=null A4) but GATED DORMANT via new RenderGate.kt + VisibilityWriteAuditLogger.kt (Spec2 §10/§11.8-5c Strict-Mode). RR-2 no-double-write PASS (legacy KSM sole live writer; dormant=live=false; doubleWriteCount==0 provable; CR4 removes KSM-drive+arm()s gate same chunk). A3: option-a extract BINDING (D4 + §9.x precedent). F-6 carried→CR-DEL (tracked). KSM thinned §11.8-5c. Live keyboard unchanged. 1099 tests green, R-7 clean. 1 own-bug fixed inline Step3 (audit live-flag). No Critical → no mid-chunk-triage. Commit 1 f78af84 + Commit 2 5765432. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR2 | B5-CR2-IMPL combined Steps 1-5 (Theme-C-R, HIGH RR-1): new SpecialTouchHandlerInstaller.kt builds 3 Spec2 §11.7 handlers verbatim (CursorSwipe/Backspace-accel-delete/Enter-Overlay); real staticHandlerInstaller replaces null. **RR-1 single-owner PASS** (no double-wire, proven 3 ways: code/ownerOf-ledger/Log.wtf-guard+test-invariant). build-but-don't-attach model (= CR1 staged-safety-net on touch axis); legacy MainButtonsController sole live owner pre/post-CR2; CR4 flips via gated attachToViews. Live keyboard unchanged. 1079 tests green, R-7 clean. Dev: build-but-dormant (RR-1); backspace onDeleteCancelled→real IME callback (G3 accel-delete-interruptible, the half F-1 dropped). 1 NTH (Java↔Kotlin cast). No Critical → no mid-chunk-triage. Commit 1 d53208c + Commit 2 590049e. | ✅ |
| 2026-05-16 | Phase 3.1 B5-CR1 | B5-CR1-IMPL combined Steps 1-5 (Theme-C-R render-cutover, MED): ImeViewBackend completion per Spec 2 §6/§9.2 — ADDITIVE/dormant (no legacy drive removed=CR4; no controller deleted=CR-DEL). +keyPressAnimator ctor (reused existing) +applyTheme +EditNumbersAnimator extracted (MainButtonsController delegates, byte-identical). A1 RECORD long-press: additive, NO architecture-conflict (OnRecordLongPress pure data; 2-mode in RecordingModule reducer from state.recording; Active/Paused→discard-stop via StopRecording effect; Idle IME-affordances→CR4 per A1). RR-1 no-double-wire PASS (RESEND-only listener = pre-CR1; RECORD/BACKSPACE→CR4). Live keyboard unchanged. 1068 tests green, R-7 clean. 1 NTH (effect-list dup, deliberate). No Critical → no mid-chunk-triage. Commit 1 3d8b9f0 + Commit 2 598534b. | ✅ |
