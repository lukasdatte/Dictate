...
transcribe or post-process in production. Block D goal ("Vollständiges Diktat am
...
- `desktop-host.md §5.1 NOTE` promises: *"erst D3 gated auf die C1-Profil-Typen
  (`ActiveProfileSource` liefert dann ein echtes Profil)"* — a **real**
  profile, i.e. full resolution, not just the ambiguity axis.
- `desktop-host.md §15 Gap 5` names the **D3-Agent** as Owner ("koordiniert mit
  dem C-Strang", `depends_on: D3→{D2,C1}`).
- Plan §2.3 (Block D acceptance) requires "Transkription+Nachbearbeitung über
  **konfiguriertes Profil**", which the current slice does not satisfy.
...
**Block E** peer credential delivery ("Credential landet beim Empfänger
ausschließlich im SecretStore", §2.7) or a future credential-entry field. So:
