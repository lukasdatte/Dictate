...
5. Spec `research/entitaetenmodell-android.md` §5.3 (L680-694, the recompute-and-compare integrity invariant) and §5.4 (L723-731, "the export file body is produced via `CanonicalJson` (byte-reproducible); … so that a receiver can **independently recompute** it").
...
This also fixes it *once* for Block E: when the peer path adds its own recompute (§5.3 "Block E tightens this for peers"), reusing the same raw-based helper keeps peer sync forward-compatible instead of re-introducing this exact bug on the cross-version wire.
