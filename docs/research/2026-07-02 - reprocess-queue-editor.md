...
### 2026-07-03 — F-001/F-003 keyboard-seam closure (the anticipated seam-switch)

- **Trigger:** Fix of the two confirmed keyboard-side reprocess-queue findings (parent catalog F-001 + F-003) — the follow-up the 2026-07-02 entry explicitly reserved ("the F-001 fix later just switches its seam from `PromptQueueSlot.fromIdsOrUnset` to an explicit list").
- **What changed:**
  - **F-001 — one send flow, staged content on the action:** the staged queue + language now ride `Action.PipelineAction.SendStaging(sessionId, queuedPromptSlots, language)` as **explicit** ID-only slots (`PromptQueueSlot.fromIds(editableQueue)`; empty = run zero prompts). The keyboard seams were widened as designed: `Effect.SubmitReprocess.queue: List<Int>` → `queuedPromptSlots: List<PromptQueueSlot>?`, same on `PipelineRunnerSubsystem.submitReprocess` + `PipelineConfigResolver.resolveReprocess`; the resolvers pass the value straight through (no `fromIdsOrUnset` re-derivation — the caller owns explicit-vs-unset). `handleReprocessSend()` is the **single submitter for both surfaces**: the catalog record button routes through a new `imeSideAffordance` RECORD-staging branch (the affordance fires before the catalog dispatch, symmetric with RESEND), the QWERTZ button keeps its direct call; the IME's duplicate direct `pipelineRunner.submitReprocess(...)` is deleted and the catalog `resolveSendStagingAction` now returns `null`. Both paths therefore get the C5 `snapshotReprocess` (model/targetApp/AutoFormatting-`totalSteps`) that the catalog path used to skip — which also kills the spurious "job busy" toast from the old double-submit race.
  - **F-003 — auto-apply priming on the catalog record path:** new `prepareCatalogAutoApplyQueueIfIdle()` in the shared RECORD/OVERLAY_RECORD affordance branch primes `PromptQueueManager.prepareAutoApplyQueue()` when recording is Idle — the main record button AND the overlay widget now front-load `autoApply=true` prompts exactly like the QWERTZ `startRecording()` path.
  - **Tests:** behavioral round-trips in `PipelineModuleTest` (SendStaging payload → `Effect.SubmitReprocess`, explicit-empty stays explicit), `ImePipelineConfigResolverTest` + `PipelineRunnerSubsystemAdapterTest` (explicit-empty survives the resolvers; `null`=unset still falls back) — compile-red on the pre-fix transport (the payload fields did not exist); `ActionResolversTest` locks the null-resolver; new `ReprocessSendFlowInvariantTest` (source-scan, same pattern as `CutoverArchitectureInvariantTest`) red-proven against the pre-fix IME source: no direct `submitReprocess`, payload-bearing `SendStaging`, `handleReprocessSend` reachable from both surfaces, auto-apply primed on ≥2 surfaces (5/5 assertions red pre-fix).
- **Deviations:** none. Note (pre-existing, unchanged): the staged queue's source of truth *before* the send-tap remains the IME mirror fields (`reprocessEditableQueue`) — moving it into orchestrator state stays a candidate for the eventual staging-UI state-machine migration; the fix bounds the mirror-field exposure to exactly one seam (`handleReprocessSend`).
